...
var discordInvite = discordInvite || function() {
    ...
    return {
        init: function(e) {
            ...
            i.title = e.title, i.introText = e.introText, 
        ...
        render: function() {
            ...
            function d() {
                if (discordCode = i.inviteCode, discordCode && "" != discordCode) {
                    ...
                    var t = '<div id="discordInvite" style="width: ' + i.width + ';"><h5 id="introText" class="noselect loadHidden">' + i.introText + '</h5><div id="discordData"><div id="serverImg" class="discordLink loadHidden virgin" style="background: rgb(54, 57, 63) repeat scroll 50% 50% / 100% 100% padding-box padding-box;"></div><div id="discordInfo"><div id="serverNameBox" class="discordLink"><span class="noselect virgin" id="serverName">' + i.title + '</span></div><div id="status" class="loadHidden"><div id="statusIndicators" class="noselect"><i id="onlineInd"></i><span id="numOnline" class="virgin">... Online</span><i id="offlineInd"></i><span id="numTotal" class="virgin">... Members</span></div></div></div><button type="button" class="discordLink virgin" id="callToAction"><div id="buttonText" class="noselect">' + i.joinText + "</div></button></div></div>",
                    ...
                    $(i.targetElement).first().append(t).attr("version", e), $.ajax({
                        ...
                        success: function(e) {
                            ...
                            var t = e.approximate_member_count.toLocaleString("en") + " Members",
                                ...
                                n = e.guild.name,
                                r = "https://cdn.discordapp.com/icons/" + e.guild.id + "/" + e.guild.icon + ".webp";
                            $("#serverName.virgin").first().removeClass('virgin').html(n), $("#serverImg.virgin").first().removeClass('virgin').css("background-image", "url(" + r + ")"), $("#numTotal.virgin").first().removeClass('virgin').html(t), $("#numOnline.virgin").first().removeClass('virgin').html(o), $(".discordLink.virgin").click(function() {
                                $("#callToAction .virgin").first().html(d).attr("id", "callToAction-clicked"), url = "https://discordapp.com/invite/" + i.inviteCode, window.open(url, "_blank")
			...
			$('.discordInviteBox').first().removeClass('discordInviteBox');
        }
    }
}();
